...
public class RestaurantDAO {
   ...
   public List<RestaurantDTO> selectAllRestaurant(String id)throws SQLException{
      ...
      //6. 조회결과 얻기
         ...
         while(rs.next()) {
            ...
            rDTO.setLat(rs.getInt("lat"));
            rDTO.setLng(rs.getInt("lng"));
   ...
   public void insertRetaurant(RestaurantDTO rDTO) throws SQLException{
		DbConn dbCon = DbConn.getInstance("jdbc/dbcp");
		...
		try {
			...
			con = dbCon.getConn();
			// 4.쿼리문 생성객체 얻기
			String insertRetaurant = "insert into restaurant(rest_num,id.rest_name,menu, info, lat,lng )values( seq_rest.nextval,?,?,?,?,?,?)";
			pstmt = con.prepareStatement(insertRetaurant);
